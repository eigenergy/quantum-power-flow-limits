...
/-- The quantitative partition supplied by the external planar separator theorem. -/
...
/-- The planar corollary, conditional on the quantitative external partition. -/
theorem corollary1_planarFromLiptonTarjan (G : WeightedGraph n m)
    ...
    5 / 18 * G.totalWeight / (Real.sqrt (8 * n) * Δ * bmax) ≤
      effectiveConditionNumber G (fun _ ↦ 1) := by
  exact planar_kappa_bound_of_lipton_partition G A X B Δ bmax partition.order
    partition.cover partition.disjoint partition.noCrossing partition.separatorSize
    partition.leftSize partition.rightSize hdegree hweight hconn
...
/-- The random planar transfer, conditional on the quantitative external partition. -/
    ...
    1 - Real.exp (-2 * ε ^ 2 * (∑ e, ∫ ω, weight e ω ∂measure) ^ 2 /
      ...
      measure.real {ω | 5 / 18 * ((1 - ε) * (∑ e, ∫ ω', weight e ω' ∂measure)) /
        ...
        effectiveConditionNumber
          (G.withWeights (fun e ↦ weight e ω) (fun e ↦ (hbounded ω e).1))
          (fun _ ↦ 1)} := by
  have hcardNat : (A ∪ X).card + B.card = n := by
    rw [← Finset.card_union_of_disjoint partition.disjoint, partition.cover,
      Finset.card_univ, Fintype.card_fin]
  have hcard : ((A ∪ X).card : ℝ) + (B.card : ℝ) = n := by
    exact_mod_cast hcardNat
  have hA : (n : ℝ) / 6 ≤ ((A ∪ X).card : ℝ) := by
    linarith [partition.rightSize]
  have hXsixth : (X.card : ℝ) ≤ (n : ℝ) / 6 :=
    partition.separatorSize.trans (sqrt_eight_mul_card_le_sixth n partition.order)
  have hunionNat : (A ∪ X).card ≤ A.card + X.card := Finset.card_union_le A X
  have hunion : ((A ∪ X).card : ℝ) ≤ (A.card : ℝ) + X.card := by
    exact_mod_cast hunionNat
  have hB : (n : ℝ) / 6 ≤ (B.card : ℝ) := by
    linarith [partition.leftSize]
  exact random_kappa_bound_of_sqrt_separator_partition measure G hm weight hmeasurable
    hindependent bmax
    hbounded A X B Δ ε partition.cover partition.disjoint partition.noCrossing
    partition.separatorSize hdegree hA hB hdenominator hε hconn
    ...
    1 - Real.exp (-2 * ε ^ 2 * (∑ e, ∫ ω, weight e ω ∂measure) ^ 2 /
      ...
      measure.real {ω | 2 * β ^ 2 * ((ℓ : ℝ) - 1) ^ 2 *
        ...
        effectiveConditionNumber
          (G.withWeights (fun e ↦ weight e ω) (fun e ↦ (hbounded ω e).1))
          (fun _ ↦ 1)} := by
  exact random_corridor_kappa_bound measure G hm weight hmeasurable hindependent bmax
    hbounded left right ℓ hℓ path pathEdges hpathEdges β ε hpathInjective hleft hright
    hdisjoint hpath hnonpath hβ hleftSize hrightSize hε hconn
