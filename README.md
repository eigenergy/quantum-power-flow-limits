...
Lemma 1, the weighted and topology only bounds in Theorem 1, the treewidth and
planar consequences in Corollary 1, the weighted and topology only corridor
bounds in Proposition 1, the random weight transfers in Proposition 2, and the
balanced worst case readout results in Proposition 3. It also proves the
grounded topology transfer and exact trace correction used by the paper.
...
The topology only separator and corridor bounds hold for every positive
weighting. For independent bounded random susceptances, the public Proposition
2 wrappers also retain the exact Hoeffding probability
...
- `Separators.lean` and `TreeDecomposition.lean` prove the weighted and topology
  only forms of Theorem 1 and its treewidth consequence.
...
- `Corridors.lean` and `Random.lean` prove the weighted, topology only, and
  random forms of Propositions 1 and 2.
...
- `Grounded.lean`, `ACPowerFlow.lean`, and `OptimalPowerFlow.lean` contain the
  grounded topology transfer, trace correction, and modeled Beyond DC block
  results.
- `experiments/` contains the pinned five case generator, retained JSON, and
  the generated table used in `main_letter_v5.tex`.
